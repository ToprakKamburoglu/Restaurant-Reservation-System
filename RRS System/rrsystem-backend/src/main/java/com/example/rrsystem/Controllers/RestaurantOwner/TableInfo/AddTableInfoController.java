...
public class AddTableInfoController {
    ...
    public ResponseEntity<?> addTable(
        ...
        boolean isTableExist = addTableInfoRepository.existsByTableNameAndRestaurantId(
                tableInfo.getTableName(), restaurant.getId());

        if (isTableExist) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Collections.singletonMap("message", "Table name is already exist"));
        }
