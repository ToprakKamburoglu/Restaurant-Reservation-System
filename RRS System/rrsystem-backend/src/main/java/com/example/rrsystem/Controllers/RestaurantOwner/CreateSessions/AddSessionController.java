...
public class AddSessionController {
    ...
    public ResponseEntity<?> addSession(
        ...
        boolean isSessionExist = addSessionRepository
                .existsByRestaurantAndSessionDateAndSessionStartAndSessionActiveness(
                        restaurant, sessionInfo.getSessionDate(), sessionInfo.getSessionStart(), 1);

        if (isSessionExist) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Collections.singletonMap("message", "Session time is already exist"));
        }
