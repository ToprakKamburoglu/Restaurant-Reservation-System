...
const EmbedReservation = () => {

const params = new URLSearchParams(window.location.search);
const apiKey = params.get("apiKey");

  const query = useQuery();
  const restaurantIdFromUrl = query.get("restaurantId");
  const [restaurantId] = useState(restaurantIdFromUrl ? Number(restaurantIdFromUrl) : 1);
    ...
    const fetchSessions = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8081/api/embedded/sessions?apiKey=${apiKey}&date=${formattedDate}`,
          {
            method: "GET",
            credentials: "include",
          }
  ...
  const handleSecondFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/embedded/create?apiKey=${apiKey}`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="embed-booking-content">
      {message.text && (
        <div className={`alert ${message.type} mt-4 m-3 p-3`}>{message.text}</div>
      )}
      <div className="embed-booking-container">
...
export default EmbedReservation;